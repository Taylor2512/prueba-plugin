...
import { Template, SchemaForUI, Size, getDynamicTemplate } from '@pdfme/common';
  ...
  useEffect(() => {
    ...
    prefillTargets.forEach((field) => {
      ...
      void applyPrefillResponse(runtimeAdapter, field, request, snapshot, currentInputRef, commitInputPatch).catch((error) => {
        if (typeof console !== 'undefined' && console.warn) {
          console.warn('[@pdfme/ui] Schema runtime prefill failed', error);
        }
      });
    });
  ...
  useEffect(() => {
    ...
    const timer = globalThis.setTimeout(() => {
      fieldSnapshots.forEach(({ schema, config }) => {
        ...
        void runRuntimeRequest(runtimeAdapter, request, '[@pdfme/ui] Schema runtime sync failed');
      });
    }, 250);
