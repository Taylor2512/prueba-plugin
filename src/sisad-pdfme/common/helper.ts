...
const normalizeCollaborativeRecipientIds = (value?: string[] | string | null): string[] => {
...
export const buildSchemaAssignments = (schemas: SchemaPageArray): SchemaAssignments => {
  ...
  schemas.forEach((page, pageIndex) => {
    page.forEach((schema) => {
      ...
      const pageKey = String(typeof pageIndex === 'number' ? pageIndex : 0);
      const recipientIds = normalizeCollaborativeRecipientIds(
        rawSchema.ownerRecipientIds || rawSchema.ownerRecipientId || '__unassigned__',
      );

      recipientIds.forEach((recipientId) => {
        if (!assignments[recipientId]) {
          assignments[recipientId] = {};
        }
        if (!assignments[recipientId][fileId]) {
          assignments[recipientId][fileId] = {};
        }
