...
export const readFile = (input: File | FileList | null): Promise<string | ArrayBuffer> =>
  new Promise((resolve, reject) => {
    ...
    fileReader.onerror = () => {
      reject(new Error('[@sisad-pdfme/schemas] File reading failed'));
    };
    ...
    if (file) {
      ...
      reject(new Error('[@sisad-pdfme/schemas] No files provided'));
    }
  });
...
export const createSvgStr = (icon: IconNode | unknown, attrs?: Record<string, string>): string => {
  if (typeof icon === 'string') {
    const trimmed = icon.trim();
    return trimmed.startsWith('<svg') ? trimmed : icon;
  }

  if (!Array.isArray(icon)) {
    try {
      const isRenderableComponent =
        Boolean(icon) && (typeof icon === 'function' || typeof icon === 'object');
      if (isRenderableComponent) {
        return renderToStaticMarkup(createElement(icon as never, attrs || {}));
      }
    } catch {
      // Fall through to the manual serializer below.
    }
