...
const runnersToPage = (
  ...
  async text(element) {
    ...
    const resolvedFont = (() => {
      const matchedFont = options.fonts && getBestFont(element.svgAttributes, options.fonts);
      if (typeof matchedFont !== 'string' && matchedFont) return matchedFont;
      return page.getFont()[0];
    })();
    const textWidth = resolvedFont.widthOfTextAtSize(text, fontSize);

    const textHeight = resolvedFont.heightAtSize(fontSize);
