...
export function byAscendingId<T extends { id: any }>(a: T, b: T) {
  return a.id - b.id;
}

export function sortedUniq<T>(array: T[], indexer: (elem: T) => any): T[] {
  const uniq: T[] = [];
...
export function pluckIndices<T>(arr: T[], indices: number[]) {
